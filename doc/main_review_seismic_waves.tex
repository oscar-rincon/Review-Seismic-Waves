...
modeling. We incially describe the inclusion and exclusion criteria used to select the articles for the review. 
...
In order to answer the research question, We included research articles that reported an improved or 
comparable computational efficiency relative to standard numerical methods. Similarly to the approach of 
\citeauthoryear{mcgreivy_weak_2024}, we considered studies that compared the computational time 
required to solve models, but restricted our focus to those within our field of interest. We also included works that 
applied physics-informed neural networks to solve inverse problems and provided this comparative analyses. In both 
cases, either a quantitative or qualitative comparison was required.

\subsection*{Search Strategy}

Based on the inclusion and exclusion criteria a search strategy was employed to identify the most relevant publications that 
address the research question outlined in the introduction. Figure \ref{fig:scheme_systematic_review} displays the flowchart 
...
\begin{tcolorbox}[colback=gray!20, colframe=gray!20, sharp corners]
\begin{quote}
\noindent\texttt{( "machine learning" OR "deep learning" OR "neural networks" ) AND ( "seismic" OR "seismology" )
 AND ( "wave equation" ) AND ( modeling OR modelling OR model OR simulation )}
\end{quote}
\end{tcolorbox}
...
The search was limited to research articles published in the subject of interest. Non-English documents were also 
excluded. The resulting list was then sorted by relevance, with the analysis limited to the first 50 results 
from both databases, which were then merged, and duplicate works removed. 

The final list was manually filtered by reading the titles and abstracts, and verifying that the publications
satisfies the inclusion and exclusion criteria. Finally, cited references were also reviewed to identify 
additional relevant articles. 

\subsection*{Result of Bibliometric Analyses}
...
\begin{table}[h!]
    ...
    \hline
    \multicolumn{1}{|l||}{\textbf{Article}} & \multicolumn{1}{c|}{\textbf{MLBM}} & \multicolumn{1}{c|}{\textbf{Wave Equation}}  & \multicolumn{1}{c|}{\textbf{Approach}} & \multicolumn{1}{c|}{\textbf{Dataset}} \\
    \hline
    \hline
