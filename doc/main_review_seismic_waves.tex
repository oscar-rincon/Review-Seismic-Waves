...
cited by over 10,000 publications \citep{Raissi2019}. Figure \ref{fig:publications_absolute_relative}.A and B shows the 
number of document published associated to machine learning, numerical methods and wave propagation modeling. 
...
\centering
    ...
    according to Scopus between 2010 and 2023. The relative number of publications is calculated as the number of
    publications with the choosen terms with respect to the total number of publications in Scopus between 2010 and 2023.
    The amount of publications associated to the term machine learning based methods (MLBM) and standard numerical methods (SNM) (A). 
    Also, amount of publications asociated wave propagation modeling (B) is shown.} 
...
of a set of observations is known as the inverse problem \citep{roth_neural_1994,Tarantola}, aiming to infer, for example, the properties of a 
...
We referred to a \textit{complementary} approach when both machine learning and standard numerical methods 
were used to perform the modeling. While an \textit{alternative} approach was considered when machine learning
methods were used as the main solver.Additionally, we have 
opened the scope to include works where physics-informed neural networks have been applied to solve inverse problems. 
Since, although, when applied to forward problems, they have shown to be both less accurate and computationally 
inefficient given the required amount of time to train the model. When applied to inverse problems, they have 
