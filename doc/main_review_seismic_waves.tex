...
\begin{tcolorbox}[colback=gray!20, colframe=gray!20, sharp corners]
    ...
    \keywords{wave equations, numerical methods, machine learning, partial differential equations, computational seismology.}   
...
Also emphasis has been placed on the application of neural networks to model seismic waves 
...
This work provides a mixed review of the advancements made in partial differential equations modeling 
through machine learning and their impact. While this area can be applied to a wide range of problems, our focus 
will be limited to the propagation of mechanical waves. The work is organized into the following sections: Section 
...
existing standard and machine learning methods used to solve the diferential equations and particularly 
...
computational time. We aimed to answer the following research question:
...
an appropriate complement or alternative to standard numerical methods?}
\end{quote}
\end{tcolorbox}

\vspace*{2mm}

We refer to a complementary approach when both machine learning and standard numerical methods are used for modeling. 
In contrast, an alternative approach is considered when machine learning methods are employed as the primary solver. 
Additionally, we have broadened our scope to include studies where physics-informed neural networks (PINNs) are applied 
to solve inverse problems. Although PINNs have generally shown to be less accurate and computationally inefficient for 
forward problems due to the time required for model training, they have demonstrated potential as alternative to 
standard numerical methods in inverse problems \citep{haghighat_physics-informed_2021,raissi_hidden_2020}. This is due 
to their versatility in handling varying amounts of data and their ability to incorporate physical laws into the model.
...
A dynamic model, such as wave propagation in a medium, describes how a system changes over time. These models typically 
rely on differential equations to characterize the system's evolution. A general formulation of the governing equation 
for a physical problem can be expressed as:
...
The search was limited to articles published between 2014 and 2024, written in English. The resulting list 
was then sorted by relevance, with the analysis limited to the first 50 results from both 
...
Additionally, the list was restricted to works that provided insights of a comparative analysis in terms of
computational time between machine learning methods and standard numerical methods in the abstract. Also,
works that applied physics-informed neural networks to solve inverse problems were included.
 
The number of articles that successfully passed the manual filtering process was \_. From these, we extracted 
key information to address the research question. The articles were summarized based on the following 
criteria: the application domain within computational seismology, the machine learning method employed, 
the standard numerical method used for comparison, and the primary outcome reported from the comparison. 
Table \ref{tab:systematic_review_surrogate} provides a summary of the main findings from the reviewed articles.
...
\begin{table}[h!]
    ...
    \hline
    \multicolumn{1}{|l||}{\textbf{Article}} & \multicolumn{1}{c|}{\textbf{Wave Equation Type}} & \multicolumn{1}{c|}{\textbf{MLBM}} & \multicolumn{1}{c|}{\textbf{Application}} & \multicolumn{1}{c|}{\textbf{Reported Efficiency}} \\
    \hline
    \hline
    \RaggedRight Example Article 1 & \RaggedRight Type 1 & \RaggedRight Method 1 & \RaggedRight Application 1 & \RaggedRight Efficiency 1 \\
    \hline
    \RaggedRight Example Article 2 & \RaggedRight Type 2 & \RaggedRight Method 2 & \RaggedRight Application 2 & \RaggedRight Efficiency 2 \\
    \hline
    \RaggedRight Example Article 3 & \RaggedRight Type 3 & \RaggedRight Method 3 & \RaggedRight Application 3 & \RaggedRight Efficiency 3 \\
    \hline
    \RaggedRight Example Article 4 & \RaggedRight Type 4 & \RaggedRight Method 4 & \RaggedRight Application 4 & \RaggedRight Efficiency 4 \\
    \hline    
    \end{tabular}
...
For physics-informed neural networks, the application domain and primary outcomes were also extracted. 
Table \ref{tab:systematic_review_PINNs} presents a summary of the key findings from these reviewed articles.

\begin{table}[h!]
    \centering
    \renewcommand{\arraystretch}{1.5}
    \footnotesize
    \begin{tabular}{|m{2.7cm}||m{2.7cm}|m{2.7cm}|m{2.7cm}|}
    \hline
    \multicolumn{1}{|l||}{\textbf{Article}} & \multicolumn{1}{c|}{\textbf{Wave Equation Type}} & \multicolumn{1}{c|}{\textbf{Application}} & \multicolumn{1}{c|}{\textbf{Main contribution}} \\
    \hline
    \hline
    \RaggedRight Example Article 1 & \RaggedRight Type 1 & \RaggedRight Application 1 & \RaggedRight Contribution 1 \\
    \hline
    \RaggedRight Example Article 2 & \RaggedRight Type 2 & \RaggedRight Application 2 & \RaggedRight Contribution 2\\
    \hline
    \RaggedRight Example Article 3 & \RaggedRight Type 3 & \RaggedRight Application 3 & \RaggedRight Contribution 3\\
    \hline
    \RaggedRight Example Article 4 & \RaggedRight Type 4 & \RaggedRight Application 4 & \RaggedRight Contribution 4 \\
    \hline    
    \end{tabular}
    \caption{Summary of the reviewed articles and their main findings.}
    \label{tab:systematic_review_PINNs}
\end{table}
