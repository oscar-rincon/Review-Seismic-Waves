...
\begin{tcolorbox}[colback=gray!20, colframe=gray!20, sharp corners]
    ...
    \begin{abstract}
        ...
        \noindent
        % Abstract content
        Numerical modeling has been crucial for addressing problems across various scientific and engineering disciplines that involve 
        ...
        have been developed and used to model wave propagation, with special emphasis on seismology. We discuss the 
        fundamentals of wave propagation modeling, standard numerical methods, and the recent advances achieved through machine 
...
wave propagation. We addressed the following research question: In which computational seismilogy applications 
have machine learning methods been an adecuate alternative to standard numerical methods in terms of 
...
The search aimed to find significant contributions and advancements in modeling wave propagation using machine learning techniques 
over the past decade (2014-2024) and limited to English as the language of publication on the Scopus database. 
Additionally, the it was complemented by Google Scholar using the same terms, time span, and language. The resulting 
list was then sorted by relevance, with the analysis limited to the first 50 results from both databases, which were then merged 
and duplicated works removed. The final list was then manually filtered by reading the titles and abstracts. Relevance to the field of
...
% Section - Conclusions
%--------------------------------------------------------------------------------------------------
\section{Conclusions}\label{sec:coclusions}
